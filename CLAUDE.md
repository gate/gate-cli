...
- **用户 CLI**：`gate-cli info` / `gate-cli news`（**41** 个 MCP 叶：`info list` / `news list`，`-h` 查 flag；README、quickstart）。**规划统一命令**：`gate-cli tool`（`list` / `call` / `describe`）；与交易 API **鉴权隔离**（勿用 `GATE_API_KEY` 作 MCP Bearer）。
