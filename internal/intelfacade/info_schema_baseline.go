...
var InfoBaselineInputSchemas = map[string]map[string]interface{}{
	...
	"info_platformmetrics_get_stablecoin_info": infoObj(map[string]interface{}{
		"symbol":         infoStr("symbol; issuance_flow: USDT|USDC; usage_structure: USDT|USDC|DAI|FDUSD|PYUSD; depeg_events: filter by depeg_asset; omit for ranked list"),
		...
		"scope":          infoStrEnum("scope", "basic", "basic", "full"),
		"sections":       infoArrStrEnum("sections; issuance_flow and/or usage_structure and/or depeg_events; requires scope=full", "issuance_flow", "usage_structure", "depeg_events"),
		"start_date":     infoStr("start_date; UTC YYYY-MM-DD; scope=full and sections set; depeg_events: default 2020-01-01"),
		"end_date":       infoStr("end_date; UTC YYYY-MM-DD; extension window end date"),
		"min_deviation":  infoNum("min_deviation; depeg_events only: filter max_deviation >= this value; range 0.001-0.2, default 0.005"),
		"review_status":  infoStrEnum("review_status; depeg_events only: candidate|approved|rejected, default approved", "approved", "candidate", "approved", "rejected"),
	}),
	...
	"info_platformmetrics_get_exchange_reserves": infoObj(map[string]interface{}{
		...
		"include_flows":   infoBool("include_flows; scope=full only; returns flows.series[] (daily inflow/outflow/netflow in native+USD)"),
		"include_events":  infoBool("include_events; scope=full only; returns events[] (large_flow events with threshold/rolling_30d_std)"),
		"start_date":      infoStr("start_date; YYYY-MM-DD; only when include_flows=true or include_events=true"),
		"end_date":        infoStr("end_date; YYYY-MM-DD; only when include_flows=true or include_events=true"),
		"event_type":      infoStrEnum("event_type; only when include_events=true; all or large_flow filter", "all", "all", "large_flow"),
		"limit":           infoIntDefaultMax("limit; only when include_flows=true or include_events=true; max flow rows", 100, 400),
	}),
	...
	"info_platformmetrics_get_chain_activity": infoObj(map[string]interface{}{
		"metric_group": infoStrEnum("metric_group (required); staking=ETH beacon, l2=L2 ops daily, btc_l2=BTC L2 protocol ecosystem", "", "staking", "l2", "btc_l2"),
		"chain":        infoStr("chain; staking: eth|ethereum; l2: base/arbitrum/optimism/linea/zksync_era/blast or empty; btc_l2: empty=btc"),
		"project":      infoStr("project; btc_l2 only: filter by project_key (stacks|rootstock|merlin|bob|bitlayer)"),
		...
		"granularity":  infoStrEnum("granularity; l2 only: day (default); ignored for staking/btc_l2", "day", "day"),
		"limit":        infoInt("limit; l2 only: max series rows returned; 0 means no extra cap"),
	}, "metric_group"),
