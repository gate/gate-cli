...
func runInfoVerifySchema(cmd *cobra.Command, args []string) error {
	...
	if len(items) == 0 {
		p.PrintError(&output.GateError{Status: 404, Label: "SCHEMA_CACHE_EMPTY", Message: "no cached schema found; run an info command with intel backend configured first"})
